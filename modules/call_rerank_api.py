"""
Remote API Call Module for Reranking

This module provides functionality to call remote LLM APIs for reranking video search results.
...
def call_rerank_api(provider, query, video_descriptions, prompt_file="reranking.md", max_retries=3, retry_delay=2, timeout=60):
    """
    Call remote API for reranking video search results
    
    Args:
        provider: Specific provider name, if None will try all providers by priority
        query: The search query for reranking
        video_descriptions: List of video descriptions to rerank
        prompt_file: Prompt template filename (default: reranking.md)
        max_retries: Maximum retry attempts
        retry_delay: Delay between retries in seconds
        timeout: Request timeout in seconds for API calls
    
    Returns:
        Reranked results as a comma-separated string of IDs
    """
    ...
    # Format prompt with query and video descriptions
    formatted_prompt = prompt_template.replace("{{query}}", query)
    formatted_prompt = formatted_prompt.replace("{{video_descriptions}}", video_descriptions)
    
    # Create a temporary prompt file with the formatted prompt
    temp_prompt_file = f"rerank_temp_{int(time.time())}.md"
